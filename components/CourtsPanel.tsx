...
interface Props {
  ...
  onUpdateMatchScore: (matchId: string, score: number | undefined, team: "team1" | "team2") => void;
...
export default function CourtsPanel({
  ...
  const selectedPlayerIds = useMemo(() => {
    ...
    roundState.matches.forEach(m => {
      m.team1.forEach(id => id && ids.add(id));
      m.team2.forEach(id => id && ids.add(id));
      if (m.byePlayerId) ids.add(m.byePlayerId);
    });
    return ids;
  }, [roundState.matches]);

  const unselectedPlayers = useMemo(() => eventPool.filter(p => !p.isSitting && !selectedPlayerIds.has(p.id)), [eventPool, selectedPlayerIds]);
  const sittingOutPlayers = useMemo(() => eventPool.filter(p => p.isSitting), [eventPool]);

  const getPlayerByeTotal = (playerId: string) => {
    const entry = standings.find(s => s.id === playerId);
    return entry ? ((entry.byeBase || 0) + (entry.byeCount || 0) + ((entry.sitOutCount || 0) * 0.5) + (entry.byeMod || 0)) : 0;
  };
  ...
  const formatByeBreakdown = (playerId: string) => {
    ...
    if (!entry) return "";
    const parts: string[] = [];
    if (entry.byeBase) parts.push(`base: ${entry.byeBase.toFixed(2)}`);
    if (entry.byeCount) parts.push(`${entry.byeCount}x BYE`);
    if (entry.sitOutCount) parts.push(`${entry.sitOutCount} sit`);
    if (entry.byeMod) parts.push(`+${entry.byeMod.toFixed(2)} late`);
    return parts.join(' + ');
  };
    ...
    return (
      <div key={match.id} className={`rounded-xl border p-4 ${match.bye ? "border-orange-200 bg-orange-50/50" : "border-green-200 bg-green-50/50"}`}>
        <div className="flex justify-between items-center mb-4">
          ...
          {!match.bye && <div className="text-xs text-gray-500">Round {currentRoundNumber}</div>}
        </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="space-y-1">
                  {team1Players.map(p => (
                    <button key={p.id} onClick={() => onSwapPlayerTeam(match.id, p.id)} className="w-full text-left rounded-lg px-3 py-2 bg-purple-100 border border-purple-200 hover:bg-purple-200 text-gray-700 transition-colors">
                      <div className="font-medium text-sm">{p.name}</div>
                      <div className="text-xs text-gray-500">{((p.duprScore ?? p.manualDuprScore) ?? '').toString()}</div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {team2Players.map(p => (
                    <button key={p.id} onClick={() => onSwapPlayerTeam(match.id, p.id)} className="w-full text-left rounded-lg px-3 py-2 bg-green-100 border border-green-200 hover:bg-green-200 text-gray-700 transition-colors">
                      <div className="font-medium text-sm">{p.name}</div>
                      <div className="text-xs text-gray-500">{((p.duprScore ?? p.manualDuprScore) ?? '').toString()}</div>
                    </button>
                  ))}
              ...
              <div className="mt-3 pt-3 border-t border-gray-200">
                ...
                <div className="flex flex-wrap gap-1">
                  {unselectedPlayers.slice(0, 6).map(p => (
                    <button key={p.id} onClick={() => onSwapPlayerTeam(match.id, p.id)} className="px-2 py-1 bg-gray-100 text-gray-600 rounded text-xs hover:bg-gray-200 transition-colors border border-gray-200">
                      {p.name} {(p.duprScore ?? p.manualDuprScore) != null ? `(${((p.duprScore ?? p.manualDuprScore)!).toFixed(1)})` : ""}
                    </button>
                  ))}
                  {unselectedPlayers.length > 6 && <span className="text-xs text-gray-400 px-2 py-1">+{unselectedPlayers.length - 6} more</span>}
                </div>
              </div>
            )}
    ...
    <section className="bg-white rounded-2xl shadow-xl p-6">
      {!roundState.active ? (
        ...
        <>
          ...
          {roundState.submitted && (<p className="text-center text-green-600 text-sm mt-2">Round submitted! Click "Start Next Round" to continue.</p>)}
        </>
      )}
    </section>
  );
