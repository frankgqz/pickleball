...
export function generateMatches(
  ...
  // Determine bye players - use FULL byeTotal (byeBase + byeCount + sitBonus + mod)
  const activeStandings = standings.filter(s => activePlayers.some(p => p.id === s.id));
  const sortedByBye = [...activeStandings].sort((a, b) => {
    // For round 1, use tempByeMap (regenerated byeBase)
    // For round 2+, use the full byeTotal including byeCount
    const aByeBase = tempByeMap?.get(a.id) ?? a.byeBase ?? 0;
    const bByeBase = tempByeMap?.get(b.id) ?? b.byeBase ?? 0;
    
    // Calculate full byeTotal: byeBase + byeCount + sitBonus + mod
    const aByeTotal = aByeBase + (a.byeCount || 0) + ((a.sitOutCount || 0) * 0.5) + (a.byeMod || 0);
    const bByeTotal = bByeBase + (b.byeCount || 0) + ((b.sitOutCount || 0) * 0.5) + (b.byeMod || 0);
    
    return aByeTotal - bByeTotal; // Lower byeTotal gets the bye
  });
